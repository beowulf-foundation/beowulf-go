...
func (encoder *Encoder) EncodeMoney(s string) error {
	...
	if r.MatchString(s) {
		...
		if err := binary.Write(encoder.w, binary.LittleEndian, perc); err != nil {
...
func (encoder *Encoder) EncodeSymbol(s string) error {
	...
	if err := binary.Write(encoder.w, binary.LittleEndian, uint32(raw.Decimals)); err != nil {
		return errors.Wrapf(err, "encoder: failed to write number: %v", raw.Decimals)
	}

	if _, err := io.Copy(encoder.w, strings.NewReader(raw.AssetName)); err != nil {
		return errors.Wrapf(err, "encoder: failed to write string: %v", raw.AssetName)
	}

	for i := byte(len(raw.AssetName)); i < 9; i++ {
		if err := binary.Write(encoder.w, binary.LittleEndian, byte(0)); err != nil {
			return errors.Wrapf(err, "encoder: failed to write number: %v", 0)
		}
	}
